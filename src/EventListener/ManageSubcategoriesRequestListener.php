...
class ManageSubcategoriesRequestListener extends EventListener {
    ...
    protected function buildDataWarehouse(Request $request) {
        $warehouse = $this->kernel->getContainer()->get('app.data.warehouse.subcategory');
        $params = new SubcategorySearchViewModel();
        $params->setCacheDisabled(true);
        if($request->get('_route') == 'manage_subcategories_particular') {
